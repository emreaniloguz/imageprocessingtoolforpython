...
class HistCanvas(FigureCanvas):
    ...
    def __init__(self, graph_type, parent=None, width=5, height=4, dpi=100,
                 facecolor=(31 / 255, 31 / 255, 31 / 255, 1)):
...
class MultiDimCanvas(FigureCanvas):
    ...
    def __init__(self, graph_type, parent=None, width=5, height=4, dpi=100,
                 facecolor=(41 / 255, 41 / 255, 41 / 255, 1)):
        fig = Figure(figsize=(width, height), dpi=dpi, facecolor=facecolor)
...
class MainWindow(QMainWindow):
    ...
    def update_pixmap(self):
        ...
        self.lower_blue = np.array(
            [self.ui.mask_1_above_slider.value(), self.ui.mask_2_above_slider.value(),
             self.ui.mask_3_above_slider.value()])
        self.upper_blue = np.array([self.ui.mask_1_below_slider.value(), self.ui.mask_2_below_slider.value(),
                                    self.ui.mask_3_below_slider.value()])
    ...
    def export_as_function(self):
        ...
        self.function_file.write(
            "min_value = np.array([" + str(self.ui.mask_3_above_slider.value()) + "," + str(
                self.ui.mask_2_above_slider.value()) + "," +
            str(self.ui.mask_1_above_slider.value()) + "])\n"
        )

        self.function_file.write("max_value = np.array([" + str(self.ui.mask_3_below_slider.value()) + "," + str(
            self.ui.mask_2_below_slider.value()) + "," +
        ...
        self.function_file.write(
            "invert_masked_img = cv2.bitwise_and(original_img, original_img, mask = invert_masked_range)\n")
